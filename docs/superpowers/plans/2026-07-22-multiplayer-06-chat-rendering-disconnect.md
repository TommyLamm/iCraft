...
> **Completed:** 2026-07-22. Automated coverage exercises the two-client chat relay, authenticated sender resolution, pose/block/disconnect data paths, avatar mesh, name projection, chat state, and remote-entity cleanup. The workspace cannot perform an interactive two-window visual inspection, so a Host + 2 Join manual presentation check remains recommended before release.
...
- [x] **Step 1: Add chat state to `State`**
  ```rust
...
- [x] **Step 2: Route `T` / `Enter` / `Esc` in `App::window_event`**
...
- [x] **Step 4: Receive chat into the ring buffer**
...
- [x] **Step 1: Render recent messages**
...
- [x] **Step 2: Render the input box when open**
...
- [x] **Step 1: Reuse skin/outfit tiles from the texture atlas**
  The avatar reuses the existing warm skin, teal shirt, and blue trousers tiles; no atlas layout or new GPU texture was required. A dedicated `player_skin.png` override remains a future cosmetic extension.

- [x] **Step 2: Add the `RemotePlayer` branch in `mob_renderer::render_mobs`**
...
- [x] **Step 3: Ensure remote players are included in the mob mesh data**
...
- [x] **Step 1: Project remote-player positions to screen space**
...
- [x] **Step 1: Handle `Disconnected` from the network drain**
...
- [x] **Step 2: Render the disconnect overlay**
...
- [x] **Step 4: Handle host-side client disconnect**
...
- [x] **Step 2: Automated end-to-end multiplayer verification (matches task 25 acceptance)**
