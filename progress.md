...
*   [ ] 初始化 `winit` 視窗與基本的遊戲主循環 (Game Loop)
*   [ ] 初始化 `wgpu` 設備 (Device)、佇列 (Queue) 與著色器 (WGSL)
*   [ ] 實作基本的 3D 相機 (Camera) 與矩陣變換上傳 (Uniform Buffer)
*   [ ] 載入方塊紋理圖集 (Texture Atlas) 並設置採樣器 (Sampler)

### 3.3. 第二階段：世界數據與網格最佳化 (Mesh)
*   [ ] 實作 `Chunk` 數據結構 ($16 \times 256 \times 16$) 與方塊定義
*   [ ] 實作 **Face Culling** (面片剪裁) 演算法以大量精簡 GPU 頂點數
*   [ ] 實作 2D/3D Noise 地形生成，產生基本的隨機山丘地貌
*   [ ] 完成單個與多個 Chunk 的靜態渲染與材質映射

### 3.4. 第三階段：玩家控制與物理碰撞
*   [ ] 實作第一人稱相機旋轉（鎖定滑鼠、讀取相對位移）
*   [ ] 實作玩家鍵盤移動（WASD、空格跳躍）
*   [ ] 實作重力與加速度公式
*   [ ] 實作 **AABB 軸對齊包圍盒** 碰撞偵測與位置推回修正，防穿牆與掉落

### 3.5. 第四階段：方塊互動與射線檢測
*   [ ] 實作 DDA 3D 射線檢測演算法，取得玩家眼睛指向的方塊位置
*   [ ] 實作滑鼠左鍵挖掘（方塊設為 Air，觸發 Chunk 網格重新生成）
*   [ ] 實作滑鼠右鍵放置（在相鄰法線方向放置方塊，觸發網格更新）
*   [ ] 實作准星 (Reticle) 或是選中方塊的外框標記

### 3.6. 驗證與最佳化
*   [ ] 撰寫單元測試驗證 AABB 碰撞與 DDA 射線正確性
*   [ ] 整合測試所有模組，在 Windows 本地編譯執行，確認無記憶體洩漏與渲染掉幀
*   [ ] 整理開發 Walkthrough 並交付
