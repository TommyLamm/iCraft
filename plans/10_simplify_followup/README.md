...
- 本波 09 號（v20 附帶項）必須與 09 的 01 **同一個 handshake bump** 落地，否則 `Container` wire gap 與每封包 `protocol_version` 要等 v21。
...
| 05 | [渲染／UI／存檔／資源死 API 與手寫 ZIP](05_dead_api_render_save_resources_zip.md) | P0 | ~-800 行 + `unicode-segmentation` | 無 | 未開始 |
| 06 | [lib 圍籬：桌面模組移出、weather／advancements 收桌面](06_lib_fence_desktop_only_modules.md) | P0 | `icraft-server` 少編 ~3,000 行 UI／lang／wgpu layout | 無 | 未開始 |
...
| 15 | [embedded 零拷貝投影：`Arc<Chunk>`、突變只套一次](15_embedded_zero_copy_projection.md) | P1 | 單機每欄少 ~3×288 KiB 拷貝＋palette 重建＋全欄 lighting | 07 | 未開始 |
| 16 | [worldgen 直接填 paletted section、Superflat、Nether 光、flatten SoA](16_worldgen_paletted_fill_and_flatten.md) | P1 | 每欄少 3×98k 陣列＋24 section 拷貝；Superflat 不再跑完整 Overworld | 無 | 未開始 |
| 17 | [lighting BFS 去重、鄰域快取、載入只 seed 邊界](17_lighting_bfs_dedupe_and_neighborhood.md) | P1 | 放／破方塊光照少 2 次 HashMap／cell；載入少 1–8 ms | 無 | 未開始 |
| 18 | [frame 熱路徑：只走 visible set、上傳批次、字型快取、刪 entity LOS worker](18_frame_hot_path.md) | P1 | 每 frame 少 O(sections) AABB、~17 次 `write_buffer`、一條 LOS 執行緒 | 09 波 04 | 未開始 |
| 19 | [`BlockType` 靜態屬性表](19_block_type_static_property_table.md) | P2 | ~-1,200 行 match；lighting／mesh 每 voxel 不再 121 臂 | 無 | 未開始 |
