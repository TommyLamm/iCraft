...
- [ ] `ServerWorld::tick` 收 `&BTreeSet<(i32,i32)>`（或可重用 buffer）作為 simulation union，由 runtime interest 提供；`occupants` 只在玩家跨欄時重建。
- [ ] `residency_keep_set` 快取直到任一 session 跨欄／改距離／換維度；resident generation counter 讓「keep == resident」O(1) 短路。
- [ ] `next_unique_entity_id` 信任單調計數器，只 probe **目標** world 與該 session hook；全掃降為 `debug_assert`。
- [ ] `loaded_chunks`／`entities` metrics 在 tick 已走過的 world 上累加；over-budget 只進 `last_tick_time_us`／`max_tick_time_us`。
- [ ] `ServerWorld::tick` 回 `Vec<WorldMutation>`（+ 待處理 dispenser actions），不再建 `AuthoritySnapshot`。
- [ ] `update_physics` 收 3×3 `&Chunk` 鄰域（與 lighting／mesh 同模式），不再每 voxel `chunks.get`。
- [ ] （最後階段，可拆）entity physics 用 Rayon 對 slice 平行：只讀 `chunks`，收集 `moved_ids` **排序後**才 `sync_entity_positions`；redstone／fluid／random tick 維持序列；checksum 測試前後一致。
- [ ] `ARCHITECTURE.md` Tick vs frame：union 由 interest 提供、物理鄰域、平行 physics 的排序套用規則。
