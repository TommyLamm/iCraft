...
- `cargo run --release --locked --bin icraft-server -- --config
  plans/04_minecraft_foundation_gap/artifacts/server.properties --world
  plans/04_minecraft_foundation_gap/artifacts/headless_once_world --bind 127.0.0.1
...
- Wall-clock dedicated soak command (duration flag is bounded automation for
  ...
  ```text
  target/release/icraft-server.exe --config plans/04_minecraft_foundation_gap/artifacts/server.properties --world plans/04_minecraft_foundation_gap/artifacts/headless_wall_soak_20260811_220121_world --bind 127.0.0.1 --port 56912 --duration-seconds 1800
  ```
