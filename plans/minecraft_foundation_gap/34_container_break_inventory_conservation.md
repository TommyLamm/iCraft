...
- [ ] 權威破壞 Chest、Furnace、Hopper、Dispenser、Dropper 前，先以完整 `ItemStack`
...
- [ ] 方塊 drop、容器內容 drops、block mutation 與 block-entity removal 形成單一可驗證 commit；
  失敗不得部分清空，duplicate/retry/stale 不得重複掉落。
- [ ] owner 與 interest observers 收到一致的 item entity 與 `BlockEntityDelta(None)`；
  observer 不得收到 owner-private session payload。
- [ ] 真 TCP Listen 與 Dedicated 各驗證至少一個非空容器，並以 authority 單元矩陣涵蓋
