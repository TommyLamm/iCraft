...
```powershell
pwsh plans/03_performance/tools/New-R9Manifest.ps1 `
  -OutputPath plans/03_performance/reports/<capture>/manifest.json `
  -SettingsPath <settings.json> -WgpuBackend dx12 `
  -Resolution 1920x1080 -RenderDistance 16

pwsh plans/03_performance/tools/Invoke-R9Matrix.ps1 `
  -Command <workload-executable> -Phase before `
  -OutputRoot plans/03_performance/reports/<capture>/before
```
...
```powershell
pwsh plans/03_performance/tools/Measure-R9Runs.ps1 `
  -InputPath plans/03_performance/reports/<capture>/before `
  -OutputPath plans/03_performance/reports/<capture>/before-summary.json `
  -ManifestPath plans/03_performance/reports/<capture>/manifest.json
```
...
results. `plans/03_performance/reports/r9-report-template.md` lists the required
