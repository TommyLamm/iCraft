...
> 狀態：Complete
> 審核修復：見 [`15_performance_audit_repair_plan.md`](15_performance_audit_repair_plan.md)；CPU/WGSL AO decode 不一致，且 meshing 仍以完整 Chunk 為 ownership，待 R6 修復。
...
- `src/chunk_render.rs:18` - `TerrainVertex`：已壓縮至 16 bytes（pos u16x3、light_ao u16、local_uv u16x2、atlas_tile u16x2）。
- `src/world.rs:1931` - `Chunk` 結構。
- `src/world.rs:2552` - `generate_mesh`：完整 Chunk mesh 生成。
- `src/world.rs:2956` - `generate_mesh_bundle`。
- `src/state.rs:861` - `MeshSnapshot`：只保存 18³ halo voxels。
- `src/state.rs:952` - `ChunkMeshResult`：generation/lifetime/revision 驗證。
- `src/state.rs:4968` - `update_chunks`：mesh 重建調度。
- `src/shader.wgsl` - terrain shader（vs_terrain 重建 world position 與 decode attributes）。

## 子任務清單

### 11.1 Packed TerrainVertex 格式
- [x] 檔案：`src/chunk_render.rs`、`src/shader.wgsl`
- 步驟：
  1. 壓縮 `TerrainVertex` 至約 16-20 bytes：
     - region-relative fixed-point/u16 position（3 × u16 = 6 bytes）
     - packed local UV（2 u16 = 4 bytes）
     - integer atlas tile（2 u16 = 4 bytes）
     - packed sky/block light、AO 與 face flags（1 u16 = 2 bytes）
  2. Shader 以 region origin 重建 world position。
  3. 確認 vertex layout 與 WGSL attribute 對齊。
- 驗收：TerrainVertex 從 36 bytes 降至 16 bytes；shader 正確重建 position。

### 11.2 邊界與精度驗證
- [x] 檔案：`src/chunk_render.rs`（測試區段）
- 步驟：
...
### 11.3 Section 級切分
- [x] 檔案：`src/world.rs`、`src/chunk_render.rs`、`src/state.rs`
- 步驟：
...
- [x] TerrainVertex 壓縮至約 16-20 bytes（16 bytes）。
- [x] 單格 block mutation 不再重建完整 Chunk mesh。
- [x] 跨 section/Chunk AO、lighting、fluid 邊界正確。
- [x] 快速連續修改同一位置時，只上傳最新 revision。
- [x] Greedy UV repeat、AO triangulation、fluid、snow、door/trapdoor 與 cross-model 結果一致。
- [x] GPU vertex upload bytes 降低（與基線比較）。
- [x] `cargo fmt --all -- --check`、`cargo check --release`、`cargo test --release` 通過（368 passed）。
