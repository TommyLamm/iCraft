...
- `plans/03_performance/performance_track.md:10-26` - 任務總覽表，R9 完成後重新驗收狀態。
- `plans/03_performance/performance_track.md:270-273` - 已知驗證限制，R9 補完。
- `plans/03_performance/baselines/2026-07-28_windows_dx12.md:7-61` - 既有視距 8 baseline，不可作為視距 16 證據。
- `plans/03_performance/15_performance_audit_repair_plan.md:335-346` - 第 6 節完成定義。
...
- [x] 檔案：`plans/03_performance/baselines/`、`plans/03_performance/reports/`（新建）
- 步驟：
...
- [x] 檔案：`plans/03_performance/reports/`
- 步驟：
  1. 視距 16 的 claims 一律以視距 16 量測，不可用視距 8 baseline 充數。
  2. 標註既有 `plans/03_performance/baselines/2026-07-28_windows_dx12.md` 為視距 8，不可作為視距 16 證據。
  3. 視距 16 量測涵蓋 8 個固定場景。
- 驗收：視距 16 claims 全部以視距 16 artifact 證明。

### 9.3 PGO 同 workload A/B
- [x] 檔案：`plans/03_performance/reports/`、`Cargo.toml`/build 設定
- 步驟：
...
### 9.6 重新驗收 01–14 文件狀態
- [x] 檔案：`plans/03_performance/performance_track.md`、`plans/03_performance/01_*.md`–`plans/03_performance/14_*.md`、`plans/03_performance/15_performance_audit_repair_plan.md`、`ARCHITECTURE.md`
- 步驟：
...
### 9.7 完成定義一致性檢查
- [x] 檔案：`plans/03_performance/15_performance_audit_repair_plan.md`、`plans/03_performance/performance_track.md`
- 步驟：
...
- `plans/03_performance/benchmarks/r9-scenes.json` 固定八個 scene ID、seed、重播步驟與
  render distance 16。
- `plans/03_performance/tools/Validate-R9Jsonl.ps1`、`Measure-R9Runs.ps1`、
  `Invoke-R9Matrix.ps1`、`New-R9Manifest.ps1` 與 `Compare-R9Pgo.ps1` 已通過
  `plans/03_performance/tools/Test-R9Tools.ps1`；測試只驗證 schema、dry-run 與 fail-closed
  gate，沒有執行 GPU/window workload。
- `plans/03_performance/reports/README.md` 與 `r9-report-template.md` 定義 raw JSONL、
  contemporaneous hardware manifest、重播指令及 PGO admission gate。
- `plans/03_performance/baselines/README.md` 明確標註既有 2026-07-28 報告為視距 8
  歷史資料，不可支持視距 16 claim。

上述 tooling 不會填入或推導性能數字。本機續作已安裝 Rust
`llvm-tools-preview`，並產出
`plans/03_performance/reports/2026-07-30-local-manifest.json`（DX12、1280×720、
...
cargo run --release   # 固定場景 1-8 before/after 量測，輸出 raw data 至 plans/03_performance/reports/
# PGO A/B（視改善門檻決定是否納入 pipeline）
git diff --check      # 確認 plans/03_performance/ 與 ARCHITECTURE.md 狀態一致、src/ 未動
```
