...
> 對應計畫：`plans/02_implementation/12_block_action_result.md`（Task 10 後續項 G1）
...
- [x] 檔案：`src/network/protocol.rs`（或新 `src/network/wire_item.rs`）
- 步驟：
...
- [x] 檔案：`src/network/protocol.rs` 測試區
...
- [x] 檔案：`src/network/protocol.rs:5`
...
- [x] 檔案：`src/network/server.rs` 測試區（參考 `:1003`）
...
- [x] 檔案：`src/network/server.rs` 測試區
...
- [x] 檔案：`src/state.rs`（drain 區，約 `:2140`/`:4034`）
- 步驟：
...
- [x] 檔案：`src/state.rs`（重構 `break_block:6486`）
...
- [x] 檔案：`src/state.rs:4034` drain 區
...
#### 12.23 更新文件
- [ ] 檔案：`ARCHITECTURE.md`、`track.md`、`plans/01_basic_gameplay/progress.md`、`plans/02_implementation/10_bug_audit.md`
...
#### 12.25 Commit
- [x] 步驟：單一 commit `feat(network): authoritative block action-result for joined clients`，只 stage 本任務檔案。
- 驗證：`git diff --check` 通過。

## 驗收條件（對應計畫驗證清單）
- [x] `ItemWire <-> ItemStack` roundtrip（含附魔、藥水、耐久、自訂名稱）。
- [x] Protocol v5 `BlockActionRequest`/`BlockActionResult` roundtrip。
- [x] 舊協議（v4）client handshake 被拒。
- [x] Host break：harvest eligibility、fortune、silk touch、XP、exhaustion、tool durability 正確。
- [x] Host place：reach + collision + support 驗證；成功 `consumed_item`。
- [x] Host 驗證失敗：`success:false`，不修改 world、不廣播。
- [x] Client 收到 `success+consumed_item`：消耗一個 selected item。
- [x] Client 收到 `success+drops`：drops 入背包；滿時 spawn 本地 drop。
- [x] Client 收到 `!success`：不消耗物品、不生成掉落物。
- [x] Host 廣播 `BlockChange` 仍正確到達所有 client（含非請求者）。
- [x] 既有 placement collision、support、authenticated-id 及 multiplayer block sync 測試不回歸。
- [x] `cargo fmt --all -- --check`、`cargo test --release`、`cargo check --release`。
- [x] 人工 Host + Join 生存模式放置（消耗物品）與破壞（獲得掉落物）。
