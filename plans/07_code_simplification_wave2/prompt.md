# iCraft 第二波代碼簡化路線 — 單 Agent 執行 Prompt
...
```text
plans/07_code_simplification_wave2/01_state_dead_gpu_pipelines_and_buffers.md
```
...
2. 開始修改前，必須完整閱讀：
   - ARCHITECTURE.md
   - plans/07_code_simplification_wave2/README.md
   - {{PLAN_FILE}}
...
- 本路線是行為 100% 不變的簡化。所有輸入、輸出、拒絕原因、revision 分配、tick checksum 必須保持。
...
- 把 plans/07_code_simplification_wave2/README.md 該列狀態改成「已完成」。
```
