...
- [x] 運行 `cargo check` 可順利編譯通過
- [x] 運行 `cargo test` 可通過所有單元測試（包含生態群系分佈及樹木放置邊界測試）
- [x] 運行 `cargo run` 進入遊戲，地形呈現 Plains、Forest、Desert、Taiga、Swamp、Mountains、Ocean 等生態且過渡平滑自然
- [x] 地圖上自然分佈並生成橡木、白樺木、雲杉木，形狀各異且跨邊界無生硬截斷
- [x] 沙漠中生成仙人掌，碰撞時玩家閃紅受傷；水邊生成甘蔗；草地散落高草、黃花與紅花
- [x] 砍伐樹幹原木後，周圍懸空的樹葉在數秒內藉由隨機 Tick 自動衰敗消失
- [x] 破壞 Tall Grass 有 12.5% 機率掉落 `Seeds`，破壞樺木/雲杉葉有機率掉落 `Apple`
