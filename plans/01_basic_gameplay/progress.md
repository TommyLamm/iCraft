...
>
> **審查硬化路線（2026-08-16）**: 5 / 15 已完成（P0 01–05） → [`../05_review_hardening/README.md`](../05_review_hardening/README.md)

### 審查硬化路線（2026-08-16）

全專案五軸 review 後的契約修復，**不是**新的 Minecraft 功能缺口。
一次只跑一份：[`../05_review_hardening/prompt.md`](../05_review_hardening/prompt.md)。

| 級 | 計劃 | 狀態 |
| --- | --- | --- |
| P0 | [01 BlockUse](../05_review_hardening/01_close_block_use_mutation.md) · [02 容器 click](../05_review_hardening/02_container_click_conservation.md) · [03 有界 decode](../05_review_hardening/03_bounded_bincode_decode.md) · [04 身份](../05_review_hardening/04_handshake_identity.md) · [05 restore](../05_review_hardening/05_fail_closed_chunk_restore.md) | 已完成 |
| P1 | [06 Embedded 表現層](../05_review_hardening/06_embedded_presentation_no_mutation.md) · [07 Join 投影](../05_review_hardening/07_join_client_projection_only.md) · [08 session 生命週期](../05_review_hardening/08_authority_session_lifecycle.md) · [09 Signed-Y](../05_review_hardening/09_signed_y_completion.md) · [10 世界生成](../05_review_hardening/10_worldgen_ore_structure_floor.md) | 待執行 |
| P2 | [11 chunk 駐留](../05_review_hardening/11_interest_chunk_residency.md) · [12 網路背壓](../05_review_hardening/12_network_ingress_backpressure.md) · [13 持久化](../05_review_hardening/13_persistence_integrity.md) · [14 桌面／lib](../05_review_hardening/14_desktop_runtime_hygiene.md) · [15 測試契約](../05_review_hardening/15_verification_contract.md) | 待執行 |
...
settings/subtitles/focus navigation, and deterministic singleplayer gameplay
workflows. Plan18 now has AuthorityCore/ServerWorld, persistence/interest,
management/transport foundations and a real dedicated runtime + two TCP client
harness. State gameplay-mutation convergence, the complete listen/dedicated
scenario rows, fault/metrics closure, soak, and GPU/audio/manual visual evidence
remain open; no manual checkbox is inferred from automated test counts.
...
| 30 | [渲染優化](./p3/30_render_optimization.md) | 🟢 已完成 | 2026-07-23 | 2026-07-23 | Section meshing、LOD 與視錐剔除已落實 |
...
- 🔧 完成 Task 12：Host Authoritative Block Action-Result 驗證與 Network Protocol v5 升級
  - 修改文件：`src/inventory.rs`, `src/network/protocol.rs`, `src/network/server.rs`, `src/network/client.rs`, `src/state.rs`, `ARCHITECTURE.md`, `track.md`, `plans/01_basic_gameplay/progress.md`, `plans/02_implementation/track_12_block_action_result.md`
...
- 🔧 完成 Task 10 後續項 G3：紅石組件元資料 (facing/delay/comparator_mode/note) 持久化
  - 修改文件：`src/redstone.rs`, `src/save.rs`, `src/state.rs`, `plans/02_implementation/10_bug_audit.md`, `ARCHITECTURE.md`, `track.md`, `plans/01_basic_gameplay/progress.md`
...
- 🔧 完成 Task 10 潛在 Bug 審計的快速收尾 pass (By Codex, sub-agent 交付已審核；最後依使用者要求停止再派 sub-agent)
  - 新增文件：`plans/02_implementation/10_bug_audit.md`
...
- ✅ 修復開啟物品欄仍會旋轉視角 (By inventory-camera sub-agent, reviewed by Codex)
  - 修改文件：`src/app.rs`, `src/state.rs`, `ARCHITECTURE.md`, `plans/02_implementation/09_inventory_camera_lock.md`, `plans/01_basic_gameplay/progress.md`, `track.md`
...
- ✅ 新增 Creative 原版式物品目錄 (By creative-inventory sub-agent, reviewed by Codex)
  - 修改文件：`src/inventory.rs`, `src/state.rs`, `src/app.rs`, `ARCHITECTURE.md`, `plans/02_implementation/08_creative_inventory.md`, `plans/01_basic_gameplay/progress.md`, `track.md`
...
- ✅ 新增可調 Weather 音量 (By rain-volume sub-agent, reviewed by Codex)
  - 修改文件：`src/audio.rs`, `src/menu.rs`, `src/state.rs`, `ARCHITECTURE.md`, `plans/02_implementation/07_weather_volume.md`, `plans/01_basic_gameplay/progress.md`, `track.md`
...
- ✅ 修復 Survival 怪物攻擊 (By combat sub-agent, reviewed by Codex)
  - 修改文件：`src/app.rs`, `src/state.rs`, `src/mob.rs`, `ARCHITECTURE.md`, `plans/02_implementation/06_survival_combat.md`, `plans/01_basic_gameplay/progress.md`, `track.md`
...
- ✅ 修復火把模型 (By torch-model sub-agent, reviewed by Codex)：加入正確 3D 地面火把
  - 修改文件：`src/world.rs`, `ARCHITECTURE.md`, `plans/02_implementation/05_torch_model.md`, `plans/01_basic_gameplay/progress.md`, `track.md`
...
- ✅ 修復方塊放置碰撞 (By placement sub-agent, reviewed by Codex)：禁止把 solid 方塊放進玩家
  - 修改文件：`src/physics.rs`, `src/state.rs`, `src/network/server.rs`, `ARCHITECTURE.md`, `plans/02_implementation/04_player_placement_collision.md`, `plans/01_basic_gameplay/progress.md`, `track.md`
...
- ✅ 新增額外功能 (By Codex)：Minecraft 式 Creative 飛行
  - 修改文件：`src/app.rs`, `src/physics.rs`, `src/state.rs`, `ARCHITECTURE.md`, `plans/02_implementation/03_creative_flight.md`, `plans/01_basic_gameplay/progress.md`, `track.md`
...
- ✅ 修復任務 #25 後續問題 (By Codex)：低延遲多人遊戲的遠端玩家移動閃現
  - 修改文件：`src/state.rs`, `src/mob.rs`, `src/network/protocol.rs`, `src/network/transport.rs`, `src/network/client.rs`, `src/network/server.rs`, `ARCHITECTURE.md`, `plans/02_implementation/02_multiplayer_smoothing.md`, `plans/01_basic_gameplay/progress.md`, `track.md`
...
- ✅ 完成任務 #30 (By Codex)：渲染優化
  ...
  - 修改文件：`src/camera.rs`, `src/main.rs`, `src/mob.rs`, `src/passive_mob.rs`, `src/shader.wgsl`, `src/state.rs`, `src/world.rs`, `ARCHITECTURE.md`, `plans/01_basic_gameplay/p3/30_render_optimization.md`, `plans/01_basic_gameplay/progress.md`, `track.md`
...
- ✅ 完成任務 #25 (By Codex)：多人遊戲 - 子任務 6/6 聊天、遠端玩家渲染與斷線處理
  - 修改文件：`src/state.rs`, `src/app.rs`, `src/mob_renderer.rs`, `src/network/server.rs`, `src/network/client.rs`, `ARCHITECTURE.md`, `plans/01_basic_gameplay/progress.md`, `docs/superpowers/plans/2026-07-22-multiplayer-06-chat-rendering-disconnect.md`
...
- 🔧 補完任務 #25 子任務 3/6 未完成驗證步驟 (By GLM-5.2)：Step 7 編譯冒煙與 Step 2 雙實例煙霧測試
  - 修改文件：`src/network/client.rs`, `docs/superpowers/plans/2026-07-22-multiplayer-03-client-bridge.md`, `plans/01_basic_gameplay/progress.md`
...
- ✅ 完成任務 Task 11：Host 遠端方塊請求 Reach 驗證
  - 修改文件：`src/state.rs`, `ARCHITECTURE.md`, `track.md`, `plans/01_basic_gameplay/progress.md`, `plans/02_implementation/10_bug_audit.md`
