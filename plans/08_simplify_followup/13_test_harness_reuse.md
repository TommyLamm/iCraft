...
- [ ] 刪 `HeadlessClient`，改用 harness `TcpClient`。
- [ ] 普通 temp 路徑改 `tcp_harness::temp_world(prefix)`；port 改 `HeldLoopback`。
- [ ] 場景專屬 `properties()` 維持本地。
- [ ] 合法 round-trip 測試可用 `encode_frame`；對抗性畸形 header 不動。
- [ ] 現有 TCP 整合測試通過。
