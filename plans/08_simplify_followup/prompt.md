# iCraft 第三波簡化路線 — 單 Agent 執行 Prompt
...
```text
plans/08_simplify_followup/01_remove_legacy_owner_simulation.md
```
...
2. 開始修改前，必須完整閱讀：
   - ARCHITECTURE.md
   - plans/08_simplify_followup/README.md
   - {{PLAN_FILE}}
...
- 不得破壞 live wire 協議的正向相容，除非該計劃明確允許刪 leftover inbound 變體。
- 不得修改歷史存檔 `0..256` 遷移語意。
...
- 把 plans/08_simplify_followup/README.md 該列狀態改成「已完成」。
