...
- `plans/performance/performance_track.md:10-26` - 任務總覽表，目前 01–11 多數標為 Complete。
- `plans/performance/performance_track.md:24-26` - 任務 12/13/14 已標 Pending，需核對其餘任務狀態。
- `plans/performance/performance_track.md:270-273` - 已知驗證限制：尚未執行 GPU/window 視覺驗證、尚未建立固定場景或 before/after 報告。
- `plans/performance/15_performance_audit_repair_plan.md:13-28` - 審核結論表，逐項 Fail/Partial 結論。
- `plans/performance/baselines/2026-07-28_windows_dx12.md:7-61` - 既有基線 artifact 範圍。
- `plans/performance/01_observability_baseline.md` 至 `plans/performance/14_build_release.md` - 各任務詳細計畫內的勾選狀態需回退。
...
- [x] 檔案：`plans/performance/performance_track.md`、`plans/performance/01_*.md`–`plans/performance/14_*.md`
- 步驟：
...
- [x] 檔案：`plans/performance/baselines/`、`plans/performance/repro/`（新建）
- 步驟：
  1. 建立 `plans/performance/repro/` 目錄，收納目前可重現失敗的測試名稱、輸入 seed 與重現步驟。
  ...
  3. 保留 `plans/performance/baselines/2026-07-28_windows_dx12.md` 既有 raw data，標註其視距 8 限制，不可作為視距 16 claims 證據。
  4. 每條 repro 註明預期修復輪次（R1–R9）與目前驗證缺口。
- 驗收：`plans/performance/repro/` 下每個審核發現都有對應條目，可由他人獨立重現。

### 0.3 記錄已知驗證缺口
- [x] 檔案：`plans/performance/performance_track.md`、`plans/performance/15_performance_audit_repair_plan.md`
- 步驟：
...
### 0.4 統一狀態用語與交叉引用
- [x] 檔案：`plans/performance/performance_track.md`、`ARCHITECTURE.md`
- 步驟：
...
- [x] 各詳細計畫內虛假 `[x]` 全改回 `[ ]`。
- [x] `plans/performance/repro/` 收納所有審核發現的重現條目。
...
# R0 為純文件回退，gate 6 以 git diff 確認僅 plans/performance/ 與 ARCHITECTURE.md 變動、src/ 未動：
git diff --stat -- src
git diff --stat -- plans/performance ARCHITECTURE.md
```
...
| 本地 Markdown links | Pass | `ARCHITECTURE.md` 與 `plans/performance/**/*.md` 相對連結均可解析 |
