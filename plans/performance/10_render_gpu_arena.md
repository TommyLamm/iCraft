...
> 狀態：Partial
> 原完成宣告時間：2026-07-28
> 審核回退：見 [`15_performance_audit_repair_plan.md`](15_performance_audit_repair_plan.md)；dimension switch teardown、handle ownership/generation 與 runtime compaction 待 R6 修復。
...
- [ ] 視距 16 的 GPU buffer object 數降低至少 90%（實際 >99%，降至 40-50 個 buffer）。
- [ ] 重複改方塊不造成 arena 洩漏或使用已釋放 range。
- [ ] 切維度及 unload 正確回收所有 allocations。
- [ ] 空 mesh 不配置 placeholder buffer。
- [ ] fragmentation 具備計算與合併機制且不阻塞 gameplay。
- [ ] F3 顯示 arena 統計。
