...
> 狀態：Partial
> 審核回退：見 [`15_performance_audit_repair_plan.md`](15_performance_audit_repair_plan.md)；CPU/WGSL AO decode 不一致，且 meshing 仍以完整 Chunk 為 ownership，待 R6 修復。
...
- [ ] TerrainVertex 壓縮至約 16-20 bytes（16 bytes）。
...
- [ ] `cargo fmt --all -- --check`、`cargo check --release`、`cargo test --release` 通過（368 passed）。
