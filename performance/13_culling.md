...
- `src/state.rs:9003` - `State::render`：目前 frustum culling 但無 occlusion。
- `src/chunk_render.rs:303` - `DrawPlan`：draw 排序與剔除。
- `src/mob_renderer.rs:170` - `render_mobs`：entity 提交。
- `src/world.rs:2552` - `generate_mesh`：meshing worker（可建立 connectivity）。
- `src/entity.rs:389` - `EntityManager`：entity 遍歷。
