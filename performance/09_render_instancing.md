...
- `src/mob_renderer.rs:170` - `render_mobs`：目前每幀在 CPU 重建所有 entity cuboid mesh。
- `src/particles.rs:97` - `compile_mesh`：目前每 particle 上傳 4 vertices + 6 indices。
- `src/entity.rs:102` - `Entity` 結構（含 `EntityType`）。
- `src/state.rs:9003` - `State::render`：entity/particle 提交點。
- `src/shader.wgsl` - shader entrypoints（需新增 instance shader variant）。

## 子任務清單

### 9.1 預建 entity prototype mesh
- [ ] 檔案：`src/mob_renderer.rs`
- 步驟：
...
- [ ] 檔案：`src/mob_renderer.rs`、`src/state.rs`、`src/shader.wgsl`
- 步驟：
...
- [ ] 檔案：`src/particles.rs`、`src/state.rs`、`src/shader.wgsl`
- 步驟：
...
- 步驟：
  ...
  3. 使用 N 個 frame-in-flight buffer 輪替。
...
- [ ] entity 幾何不再每 frame 在 CPU 重建。
- [ ] 4,096 particles 不再上傳 4 vertices + 6 indices/particle。
- [ ] draw call 數隨 model 種類數，不隨 entity 數增長。
- [ ] 動畫、name tag、burning、第三人稱和 dropped item 視覺保持一致。
- [ ] 實體壓力場景的 CPU mesh build / upload 時間降低至少 70%（與基線比較）。
