...
- `src/save.rs:475` - `SaveManager` 結構。
- `src/save.rs:477` - `region_cache: HashMap`（無上限）。
- `src/save.rs:480` - `compress_bytes`（Zlib 壓縮）。
- `src/save.rs:528` - `SaveManager::new`。
- `src/save.rs:653` - `save_chunk`。
- `src/save.rs:657` - `save_chunk_in`（每存一個 Chunk 重寫整個 region）。
- `src/state.rs:4481` - `trigger_background_save`（主線程壓縮全部 Chunk）。
...
- [ ] 檔案：`src/save.rs`、`src/chunk_manager.rs`、`src/state.rs`
- 步驟：
...
- [ ] 檔案：`src/state.rs`、`src/menu.rs`、`src/app.rs`
- 步驟：
...
- [ ] 現有存檔格式保持可讀；舊存檔可正常載入。
- [ ] region cache 有 byte/entry 上限。
- [ ] autosave 期間主線程 p95 改善（與任務 1 基線比較）。
