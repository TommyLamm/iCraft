...
- [ ] 檔案：`src/state.rs`、`src/chunk_schedule.rs`
- 步驟：
...
- [ ] 檔案：`src/state.rs`、`src/culling.rs`
- 步驟：
...
- [ ] runtime 無直接 `ChunkMesh::mark_dirty` 呼叫（白名單除外）。
- [ ] stale connectivity 在 mesh dirty 時立即 invalid 並 fail-open。
- [ ] dirty queue 為持久 priority queue，不每幀重建排序完整 Vec。
- [ ] mutation -> scheduler -> worker -> visible mesh 整合測試通過。
