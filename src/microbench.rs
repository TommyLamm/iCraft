...
//!
//! The library copy compiles only under `cfg(test)` or feature `harness`.
//! The desktop binary keeps its own `mod microbench` and `--microbench`
//! entry so `cargo run -- --microbench` does not need the lib feature.
//!
