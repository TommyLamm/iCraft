...
fn legacy_metadata_preserves_saved_hardcore_and_creation_options() {
    let unique = SystemTime::now()
        ...
        .as_nanos();
    for (case, level_hardcore, rules_hardcore) in [("level", true, false), ("rules", false, true)] {
...
fn legacy_settings_without_weather_volume_use_reduced_default() {
    let settings =
        GameSettings::from_file_contents("master_volume:0.8\nsound_volume:0.6\nmusic_volume:0.2\n");
...
fn world_path_guard_rejects_escape_and_symlink_roots() {
    ...
    let created = try_create_world_link(&outside, &link) || try_create_world_link(&inside, &link);
    if created {
        assert!(
