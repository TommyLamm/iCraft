...
/// Protocol v20 adds authority-owned player item drops. Singleplayer,
/// listen-server and dedicated clients now use the same inventory debit and
/// dropped-entity spawn transaction instead of creating renderer-only items.
...
pub enum GameplayOperation {
    ...
    /// Throw an exact inventory debit from the authenticated player pose.
    /// The rich source prevents delayed/replayed input from removing a newer
    /// stack that later occupied the same slot.
    DropItem {
        source: SlotRefWire,
        look_milli: [i16; 3],
    },
