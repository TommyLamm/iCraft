...
/// Protocol v14 adds a single authoritative gameplay request envelope to the stable
...
impl GameplayRequest {
    ...
    pub fn validate_bounds(&self) -> Result<(), RejectReason> {
        ...
        let string_len = match &self.operation {
        ...
        if string_len > MAX_REQUEST_STRING_BYTES.max(MAX_COMMAND_BYTES) {
            return Err(RejectReason::StringTooLong);
        }
        if self.dimension > 2 {
